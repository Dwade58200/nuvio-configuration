...
Session du 16 septembre 2026 : passe ergonomie/qualité sur la page GitHub
de réglage des backdrops, seul fichier touché.

- **Réinitialisation par champ** (double-clic sur un curseur) sans
  toucher aux autres réglages.
- **Unités "px" ajoutées** là où elles manquaient (tuile, écart, arrondi,
  flou).
- **Info-bulles** sur les réglages les moins intuitifs (décalage cascade,
  inclinaison, flou de la lueur).
- **Nouveau réglage "Ratio du canvas final"** (16:9/21:9/4:3/1:1),
  rattrapant le flag `--ratio-canvas` déjà présent côté script mais
  jusque-là invisible dans l'aperçu de l'outil.
- **Confirmation** avant d'écraser ou supprimer un preset.
- **Export/import des presets en JSON**, pour sortir de la limite
  "propre à ce navigateur" du `localStorage`.
- **Retour visuel pendant la génération réelle** ("⏳ Génération…") sur
  gros lot d'images.
- **Contour de focus clavier** explicite sur tous les contrôles.

JS revalidé (`node --check`), balises HTML rééquilibrées ; non testé
dans un vrai navigateur (pas d'environnement graphique disponible ici) --
à valider visuellement avant merge, surtout le redimensionnement du
